...
function speak(text, tone) {
  ...
  // Select voice by tone & user toggle
  const voices = speechSynthesis.getVoices();

  // Simple heuristic for male/female voices (can be customized)
  // Male deep voice for dark tone, female soft voice for light tone

  // Check user preference toggle
  const voiceToggle = document.getElementById('voiceToggle');
  const useMaleVoice = voiceToggle && voiceToggle.checked;

  let selectedVoice;

  if (tone === "dark") {
    // Prefer male voice
    selectedVoice = voices.find(v => /male|man|boy|deep/i.test(v.name)) || voices.find(v => v.name.toLowerCase().includes('male')) || voices.find(v => v.gender === 'male');
    if (!selectedVoice && useMaleVoice) selectedVoice = voices[0]; // fallback
  } else {
    // Prefer female voice
    selectedVoice = voices.find(v => /female|woman|girl|soft/i.test(v.name)) || voices.find(v => v.name.toLowerCase().includes('female')) || voices.find(v => v.gender === 'female');
    if (!selectedVoice && !useMaleVoice) selectedVoice = voices[0]; // fallback
  }

  // If user toggled voice, override:
  if (voiceToggle) {
    if (useMaleVoice) {
      selectedVoice = voices.find(v => /male|man|deep/i.test(v.name)) || voices[0];
    } else {
      selectedVoice = voices.find(v => /female|woman|soft/i.test(v.name)) || voices[0];
    }
  }
  ...
  utterance.rate = 0.85;
  utterance.pitch = tone === "dark" ? 0.7 : 1.0;

  speechSynthesis.cancel(); // Cancel previous utterance if any
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // Preload voices so speechSynthesis.getVoices() works on some browsers
  if ('speechSynthesis' in window) {
